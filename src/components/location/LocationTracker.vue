...
const initializeMap = async () => {
    ...
    if (map) map.remove()
    map = L.map('map', { preferCanvas: true }).setView([centerLat, centerLng], 13)
    ...
    L.marker(
        ...
        { icon: blueIcon, title: 'Sua Localização' }
    ).addTo(map).bindPopup('<strong>Sua Localização</strong>')
    L.marker(
        [eatlyHQ.latitude, eatlyHQ.longitude],
        { icon: redIcon, title: 'Sede Eatly' }
    ).addTo(map).bindPopup('<strong>Sede Eatly</strong><br>Joinville, SC')
    map.invalidateSize()
    await getRoute()
}
        ...
        <div class="container">
            <header class="header">
                <div class="header-content">
                    <h1 class="title">Sede Eatly</h1>
                    <p class="subtitle">Encontre a rota até a sede</p>
                </div>
            </header>
            <main class="main-content">
                    ...
                    <div class="info-card">
                        ...
                        <div class="card-body">
                            <div class="location-item">
                                ...
                                <span class="value">{{ userLocation.latitude.toFixed(6) }}°</span>
                            </div>
                            <div class="location-item">
                                <span class="label">Longitude</span>
                                <span class="value">{{ userLocation.longitude.toFixed(6) }}°</span>
                            </div>
                            <div class="location-item">
                                <span class="label">Precisão</span>
                                <span class="value">{{ userLocation.accuracy.toFixed(2) }} m</span>
                            </div>
                        </div>
                    </div>
                    <div v-if="routeInfo" class="info-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="location-item">
                                ...
                                <span class="value">{{ formatTime(routeInfo.duration) }} de carro</span>
                            </div>
                        </div>
                    </div>
                    <div class="info-card">
                ...
                <div v-else class="empty-state">
                    <h2>Comece aqui</h2>
                    <p>Clique no botão abaixo para compartilhar sua localização e ver a rota até a sede do Eatly.</p>
